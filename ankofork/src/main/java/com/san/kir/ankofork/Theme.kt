...
fun Resources.Theme.attr(@AttrRes attribute: Int): TypedValue {
    ...
    require(resolveAttribute(attribute, typedValue, true)) { "Failed to resolve attribute: $attribute" }
    return typedValue
}
...
fun Resources.Theme.color(@AttrRes attribute: Int): Int {
    ...
    require(!(attr.type < TypedValue.TYPE_FIRST_COLOR_INT || attr.type > TypedValue.TYPE_LAST_COLOR_INT)) { "Attribute value type is not color: $attribute" }
...
@Dimension(unit = Dimension.PX)
inline fun Fragment.dimenAttr(@AttrRes attribute: Int): Int = requireActivity().dimenAttr(attribute)

@ColorInt
inline fun Fragment.colorAttr(@AttrRes attribute: Int): Int = requireActivity().colorAttr(attribute)

inline fun Fragment.attr(@AttrRes attribute: Int): TypedValue = requireActivity().attr(attribute)
